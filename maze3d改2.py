...
MAZE_FILE = MEM_DIR / "maze_state_quantum_grover_fused_2d_3d.json"
...
class LocalGroverChooser3D:
    ...
    def _measure_counts(self, good_qbits_list):
        ...
        job = self.backend.run(qc, shots=GROVER_SHOTS, seed_simulator=seed)
        result = job.result()
    ...
    def choose_direction(self, prev, cur, maze: Maze3D, visit_count, recent_path):
        ...
        min_cost = min(cost for _, cost, _ in scored)
        good_dirs = [d for d, cost, _ in scored if abs(cost - min_cost) < 1e-9]

        counts = self._measure_counts([DIR_TO_QBITS[d] for d in good_dirs])
...
class App:
    ...
    # --------------------------------------------------------
    def build_ui(self):
        self.root.title(f"{W}x{H}x{D} 多層 2D / 可旋轉 3D 量子迷宮（Grover 版）")
